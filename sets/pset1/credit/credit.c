...
int main()
{
   ...
   long number;
    ...
    //Find wich credit card the number is
    int starting_number = number/pow(10,(digits-2));
    if((starting_number == 34 || starting_number == 37) && digits == 15)
    {
        printf("AMEX\n");
        return 0;
    }if(starting_number >= 51 && starting_number <= 55 && digits == 16)
    {
        printf("MASTERCARD\n");
        return 0;
    }if(starting_number / 10 == 4 && (digits == 16 || digits == 13))
    {
        printf("VISA\n");
        return 0;
    }else{
